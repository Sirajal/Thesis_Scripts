...
#Reads in the salmon file and make table to show samples
#first line list all output folder salmon disrectory
...
#files_in make files for salmon
#name files for salmon sample name
#import salmon files
files_in = paste0(pheno_in$path,"/abundance.h5")
names(files_in) = pheno_in$sample
txi = tximport(files_in, type = "salmon" , tx2gene = tx2gene, varReduce = T)
...
for(i in 1:length(foldchange_parameters)) {
  ...
  write_csv(res_first, path = paste0("../Results/salmon_Gene/DE_GeneLevel_salmon_Results_Embryo_",fc_name,".csv"))
  ...
  res_second = as.data.frame(results(dds_fit, contrast = c("Var", "Heart_Mutant", "Heart_WT"))) %>%
       ...
       left_join(anno)
  #make result into excel csv
  write_csv(res_second, path = paste0("../Results/salmon_Gene/DE_GeneLevel_salmon_Results_Heart_",fc_name,".csv"))
}
...
write_csv(GO.wall.sig, path = "../Results/salmon_Gene/GOseq/GO_Terms_FC1.5.csv")
...
GO.wall$padj = p.adjust(GO.wall$over_represented_pvalue,  method = "BH")
GO.wall.sig  = GO.wall %>% filter(padj < 0.05) %>% arrange(padj)
write_csv(GO.wall.sig, path = "../Results/salmon_Gene/GOseq/kegg_Terms_FC1.5.csv")
