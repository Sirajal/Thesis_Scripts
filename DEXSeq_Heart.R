...
pheno.in = list.files("../Data/DEXSeq/", full.names = T) %>% # List the DEXSeq output files
  ...
  mutate(Tissue = ifelse(grepl("heart", File), "Heart", "Heart"), # Make a column that gets the type of tissue
  ...
  filter(Tissue == sample.in) #Only Heart
...
# make output plots as manual shows
...
anno = getBM(attributes = c("ensembl_gene_id",
                                "hgnc_symbol", "description", "zfin_id_symbol"),
                 filters = "ensembl_gene_id",
                 values = dexseq_significant$ensembl_gene_id,
                 mart = ensembl)
...
write_csv(dexseq_significant, path = "../Results/DEXSeq_Heart/dexseq_results.csv")
